...
void takeInput(char *dir, int first, Capacity *capacity)
{
	...
	if (first == 1)
	{
		...
		scanf("%d", &temp);
		capacity->port = (uint16_t)temp;

		printf("Enter ear capacity : ");
		scanf("%d", &temp);
		capacity->earCap = (uint8_t)temp;

		printf("Enter mouth capacity : ");
		scanf("%d", &temp);
		capacity->mouthCap = (uint8_t)temp;

		fwrite(&capacity->port, sizeof(uint16_t), 1, inputUI);
		fwrite(&capacity->earCap, sizeof(uint8_t), 1, inputUI);
		fwrite(&capacity->mouthCap, sizeof(uint8_t), 1, inputUI);
		fclose(inputUI);
	}
	else
	{
