        ...
        if (eb != NULL)
        {
            ...
            if (n > 0)
            {
                ...
                if (type == MSG_FIRST)
                {
                    ...
                    //     continue;
                    // }
                    // uint8_t firstType = MSG_FIRST;
                    // fwrite(&firstType, sizeof(uint8_t), 1, bm);
                    // fwrite(&myPort, sizeof(uint16_t), 1, bm);
                    // fwrite(&myEarCap, sizeof(uint8_t), 1, bm);
                    // fwrite(&myMout;hCap, sizeof(uint8_t), 1, bm);
                    // fclose(bm);
